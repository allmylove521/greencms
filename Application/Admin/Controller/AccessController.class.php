...
class AccessController extends AdminBaseController {
	...
	private function getRoleListOption($info = array()) {
		...
		if (! empty ( $info )) {
			...
			foreach ( $list as $v ) {
				...
				$disabled = $v ['status'] == 0 ? ' disabled="disabled"' : "";
				$selected = $v ['id'] == $info ['user_role'] ['role_id'] ? ' selected="selected"' : "";
				$info ['roleOption'] .= '<option value="' . $v ['id'] . '"' . $selected . $disabled . '>' . $v ['name'] . '</option>';
			}
			
			foreach ( $list_dp as $v ) {
				$selected = $v ['id'] == $info ['user_discipline_item'] ? ' selected="selected"' : "";
				$info ['dpOption'] .= '<option value="' . $v ['id'] . '"' . $selected . '>' . $v ['name'] . '</option>';
			}
		} else {
			foreach ( $list as $v ) {
			...
			foreach ( $list_dp as $v ) {
				$info ['dpOption'] .= '<option value="' . $v ['id'] . '">' . $v ['name'] . '</option>';
			}
		}
		
		if (1 == $info ['user_department_level']) {
			$info ['departOption'] .= '<option value="' . '1' . '"' . ' selected ' . '>' . '高一' . '</option>';
			$info ['departOption'] .= '<option value="' . '2' . '"' . '>' . '高二' . '</option>';
			$info ['departOption'] .= '<option value="' . '3' . '"' . '>' . '高三' . '</option>';
		} else if (2 == $info ['user_department_level']) {
			
			$info ['departOption'] .= '<option value="' . '1' . '"' . '>' . '高一' . '</option>';
			$info ['departOption'] .= '<option value="' . '2' . '"' . ' selected ' . '>' . '高二' . '</option>';
			$info ['departOption'] .= '<option value="' . '3' . '"' . '>' . '高三' . '</option>';
		} else  {
			
			$info ['departOption'] .= '<option value="' . '1' . '"' . '>' . '高一' . '</option>';
			$info ['departOption'] .= '<option value="' . '2' . '"' . '>' . '高二' . '</option>';
			$info ['departOption'] .= '<option value="' . '3' . '"' . ' selected ' . '>' . '高三' . '</option>';
		}
	...
	public function addUserHandle() {
		...
		if ($i != '') {
			...
			$user = array (
					...
					'user_tel_number' => I ( 'post.user_tel_number' ),
					'user_age' => I ( 'post.user_age' ),
					'user_sex' => I ( 'post.user_sex' ),
					
					'user_discipline_item' => I ( 'post.user_discipline_item' ),
					'user_department_level' => I ( 'post.user_department_level' ),
					'user_identity_card' => I ( 'post.user_identity_card' ),
					'user_political_status' => I ( 'post.user_political_status' ),
					'user_first_education_school' => I ( 'post.user_first_education_school' ),
					'user_high_education_school' => I ( 'post.user_high_education_school' ),
					
					'user_first_education_level' => I ( 'post.user_first_education_level' ),
					'user_high_education_level' => I ( 'post.user_high_education_level' ),
					'user_nation' => I ( 'post.user_nation' ),
	...
	public function changeRole() {
		if (IS_POST) {
			...
			if ($res ['status'] == 1)
				$this->success ( "设置成功", U ( "Admin/Access/roleList" ) );
			else
				$this->error ( "设置失败，请重试" );
			die ();
			header ( 'Content-Type:application/json; charset=utf-8' );
